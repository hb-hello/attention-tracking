...
# image1 = cv2.imread("ClassApp/models/c1.jpg")
...
def getPoseAttention(image1, frame_counter):
    logging.debug('inside getPoseAttention')
    frameWidth = image1.shape[1]
    frameHeight = image1.shape[0]
    logging.debug(f'{frameWidth}, {frameHeight}')
    ...
    # inWidth = 368
    logging.debug(f'Input for model: {inHeight}, {inWidth}')

    inpBlob = cv2.dnn.blobFromImage(image1, 1.0 / 255, (inWidth, inHeight),
                                    (0, 0, 0), swapRB=False, crop=False)
    
    logging.debug(inpBlob.shape)
    # print(inpBlob)
    ...
    logging.debug("Time Taken in forward pass = {}".format(time.time() - t))
    ...
    for person_data in personwise_data:
        logging.debug('person data ' + str(person_data))
